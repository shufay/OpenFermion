...
class JelliumTest(unittest.TestCase):
    ...
    def test_jordan_wigner_dual_basis_jellium(self):
        # Parameters.
        grid = Grid(dimensions=2, length=2, scale=1.)
        spinless_set = [True]
    ...
    def test_jordan_wigner_dual_basis_jellium_constant_shift(self):
        length_scale = 0.6
        grid = Grid(dimensions=2, length=2, scale=length_scale)
        spinless_set = [True, False]
    ...
    def test_nonperiodic_potential_integration(self):
        ...
        for length in [3]:
            grid = Grid(dimensions=2, length=length, scale=1.)
            spinless = True
            
            # min period cutoff is scale * [( length - 1 )/length] * [1/( length - 1 )] == [ scale/length ]
            period_cutoff = grid.volume_scale() ** (1. / grid.dimensions) / 2.
        
            momentum_potential = plane_wave_potential_v2(grid, spinless=spinless, e_cutoff=None, 
